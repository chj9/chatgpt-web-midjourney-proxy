...
import {
  canBase64Model, canVisionModel, chatSetting, checkDisableGpt4, file2blob, getHistoryMessage
  , getInitChat, getSystemMessage, isCanBase64Model, isTTS, localSaveAny, mlog, subModel,
  subTTS,
  whisperUpload,
} from '@/api'
  ...
  if (n == 'gpt.submit' || n == 'gpt.whisper') {
    ...
    if (dd.fileBase64 && dd.fileBase64.length > 0) {
      if (!canVisionModel(model))
        model = canBase64Model(model)// model='gpt-4-vision-preview';
